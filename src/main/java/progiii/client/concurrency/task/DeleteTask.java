...
public class DeleteTask implements Runnable {
    ...
    @Override
    public void run() {
        FutureTask<Boolean> pullDelayer = new FutureTask<>(() -> Model.getInstance().pausePullService(2));
        ...
        if (response.getType() == ResponseType.ERROR) {
            Error finalResponse = (Error) response;
            ...
            Platform.runLater(() -> {
                MainController.getInstance().shoStatusMsg("Emails deleted", Color.GREEN);
